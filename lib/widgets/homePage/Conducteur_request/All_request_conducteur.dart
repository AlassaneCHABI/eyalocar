...
import 'package:eyav2/widgets/homePage/Conducteur_request/Voiture_liste.dart';
...
class R_conducteurListingWidget extends StatefulWidget {

  String token;
  R_conducteurListingWidget({Key? key,required this.token}) : super(key: key);

  @override
  _CarsListingWidgetState createState() => _CarsListingWidgetState();
}

class _CarsListingWidgetState extends State<R_conducteurListingWidget> {
  TextEditingController? textController;
  final scaffoldKey = GlobalKey<ScaffoldState>();
  bool isSearchStarted = false;

  List<Request_conducteur> searchedVoitures = [];
  final List<Request_conducteur> voiture = [];


  void initState() {
    // TODO: implement initState
    super.initState();
    function.getsearchconducteur(widget.token).then((value){
      if(value!=null){
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text("Demandes de conducteur", style: TextStyle(fontFamily: "Ubuntu", fontSize: 22, fontWeight: FontWeight.bold),),
        backgroundColor: Colors.white,
        elevation: 0,
      ...
      body:voiture.length==0?Center(child: CircularProgressIndicator(),):  Column(
        ...
        children: [
         /* Padding(
          ...
          Expanded(
            child: ConducteurList(
              conducteur: isSearchStarted ? searchedVoitures: voiture,
            ),
          ),
        ],
      ),
